    ...
    <div className="p-4 bg-black/40 border-t border-white/5 space-y-4 select-none">
      <div className="flex items-center justify-between">
        <div className="flex gap-4">
          ...
          <PropInput label="Reload" value={data.reload_time} onChange={v => updateWand(slot, { reload_time: v })} />
          <PropInput label="Wait" value={data.fire_rate_wait} onChange={v => updateWand(slot, { fire_rate_wait: v })} />
          ...
          {Array.from({ length: Math.max(data.deck_capacity, 20) }).map((_, i) => {
            ...
            return (
              <div
                ...
                className={`
                    aspect-square rounded border flex items-center justify-center relative group/cell transition-transform active:scale-95
              ...
              >
                {isHovered && (
                  <div className={`absolute top-0 bottom-0 w-0.5 bg-indigo-400 z-50 animate-pulse ${hoveredSlot.isRightHalf ? 'right-0' : 'left-0'}`} />
                )}
                {spell ? (
